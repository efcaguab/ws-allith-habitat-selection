...
# acoustic lags
qsub -N pad -hold_jid prepare_a -V -cwd -j y -S /bin/bash -m a -M efc29@uclive.ac.nz -r yes -l mem_free=8G,h_vmem=8G runRscript.sh ./code/02_calculate_lags.R --window_length $WL
...
## VISUAL DATA ----
# visual lags
qsub -N pvd -hold_jid prepare_a -V -cwd -j y -S /bin/bash -m a -M efc29@uclive.ac.nz -r yes -l mem_free=8G,h_vmem=8G runRscript.sh ./code/11_prepare_data_visual.R --window_length $WL
# random effect models
...
# Move data
find ./data/processed -type f -print0 | xargs -0 mv -t $DIR
