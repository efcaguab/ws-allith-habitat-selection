...
ma.r.01 <- . %>% gamm4 (present ~ s (week.2, bs = "cc", k = 4) + s (lag, k = 4) + hours + sex + size, family = "binomial", data = .)
ma.r.02 <- . %>% gamm4 (present ~ s (week.2, bs = "cc", k = 4) + s (lag, k = 4) + hours + sex + size, family = "binomial", data = ., random= ~(1|id))
